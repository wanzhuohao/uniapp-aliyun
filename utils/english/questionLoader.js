...
export function getLetters(grade) {
  assertAvailableLearningGrade(grade)
  return lettersData.map(l => ({ ...l, grade, _id: `letter_${l.upper}` }))
}

export function getWordsByTheme(grade, themes) {
  assertAvailableLearningGrade(grade)
  ...
  for (const t of list) {
    ...
    for (const it of items) {
      out.push({ ...it, grade, theme: t, _id: `word_${t}_${it.word}` })
    }
  }
  return out
}
